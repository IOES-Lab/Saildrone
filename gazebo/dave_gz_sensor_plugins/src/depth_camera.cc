...
public:
  bool SaveImage(
    const float * _data, unsigned int _width, unsigned int _height,
...
public:
  bool ConvertDepthToImage(
    const float * _data, unsigned char * _imageBuffer, unsigned int _width, unsigned int _height);
...
public:
  gz::rendering::DepthCameraPtr depthCamera;
...
public:
  float * pointCloudBuffer = nullptr;

  /// \brief xyz data buffer.
public:
  float * xyzBuffer = nullptr;
...
public:
  ...
  /// is generated
public:
  gz::common::EventT<void(const gz::msgs::Image &)> imageEvent;
...
public:
  gz::common::ConnectionPtr pointCloudConnection;
...
public:
  std::uint64_t saveImageCounter = 0;
...
//////////////////////////////////////////////////
bool MultibeamSonarSensorPrivate::SaveImage(
  const float * _data, unsigned int _width, unsigned int _height,
  common::Image::PixelFormatType /*_format*/)
{
  ...
  std::string filename = this->saveImagePrefix + std::to_string(this->saveImageCounter) + ".png";
  ++this->saveImageCounter;

  localImage.SetFromData(imgDepthBuffer, _width, _height, common::Image::RGB_INT8);
  localImage.SavePNG(common::joinPaths(this->saveImagePath, filename));
...
//////////////////////////////////////////////////
bool MultibeamSonarSensor::Init() { return this->CameraSensor::Init(); }
...
bool MultibeamSonarSensor::Load(const sdf::Sensor & _sdf)
{
  ...
  this->dataPtr->pub = this->dataPtr->node.Advertise<msgs::Image>(this->Topic());
  if (!this->dataPtr->pub)
  {
  ...
  gzdbg << "Depth images for [" << this->Name() << "] advertised on [" << this->Topic() << "]"
        << std::endl;
  ...
  if (_sdf.CameraSensor()->Triggered())
  {
    ...
    {
      triggerTopic = transport::TopicUtils::AsValidTopic(this->Topic() + "/trigger");
    }
  ...
  this->dataPtr->pointPub =
    this->dataPtr->node.Advertise<msgs::PointCloudPacked>(this->Topic() + "/points");
  if (!this->dataPtr->pointPub)
  {
    gzerr << "Unable to create publisher on topic[" << this->Topic() + "/points" << "].\n";
    return false;
  }

  gzdbg << "Points for [" << this->Name() << "] advertised on [" << this->Topic() << "/points]"
        << std::endl;
...
bool MultibeamSonarSensor::CreateCamera()
{
  ...
  for (const auto & [noiseType, noiseSdf] : noises)
  {
    ...
    if (noiseSdf.Type() == sdf::NoiseType::GAUSSIAN)
    {
      ...
      // use bias parameters.
      if (!math::equal(noiseSdf.Mean(), 0.0) || !math::equal(noiseSdf.StdDev(), 0.0))
      {
        this->dataPtr->noises[noiseType] = ImageNoiseFactory::NewNoiseModel(noiseSdf, "depth");

        std::dynamic_pointer_cast<ImageGaussianNoiseModel>(this->dataPtr->noises[noiseType])
          ->SetCamera(this->dataPtr->depthCamera);
      }
    }
  ...
  this->UpdateLensIntrinsicsAndProjection(this->dataPtr->depthCamera, *cameraSdf);
  ...
  this->dataPtr->depthConnection = this->dataPtr->depthCamera->ConnectNewDepthFrame(std::bind(
    &MultibeamSonarSensor::OnNewDepthFrame, this, std::placeholders::_1, std::placeholders::_2,
  ...
  msgs::InitPointCloudPacked(
    this->dataPtr->pointMsg, this->OpticalFrameId(), true,
  ...
  this->dataPtr->pointMsg.set_row_step(this->dataPtr->pointMsg.point_step() * this->ImageWidth());

  return true;
}

/////////////////////////////////////////////////
void MultibeamSonarSensor::OnNewDepthFrame(
  ...
  const std::string & _format)
{
  ...
  common::Image::PixelFormatType format = common::Image::ConvertPixelFormat(_format);
  ...
  {
    this->dataPtr->SaveImage(_scan, _width, _height, format);
  }
}

/////////////////////////////////////////////////
void MultibeamSonarSensor::OnNewRgbPointCloud(
  const float * _scan, unsigned int _width, unsigned int _height, unsigned int _channels,
  const std::string & /*_format*/)
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  ...
  unsigned int pointCloudBufferSize = pointCloudSamples * _channels * sizeof(float);
...
bool MultibeamSonarSensor::Update(const std::chrono::steady_clock::duration & _now)
{
  ...
  if (this->HasPointConnections() && !this->dataPtr->pointCloudConnection)
  {
    this->dataPtr->pointCloudConnection =
      this->dataPtr->depthCamera->ConnectNewRgbPointCloud(std::bind(
        &MultibeamSonarSensor::OnNewRgbPointCloud, this, std::placeholders::_1,
  ...
  msg.set_step(width * rendering::PixelUtil::BytesPerPixel(rendering::PF_FLOAT32_R));
  ...
  *msg.mutable_header()->mutable_stamp() = msgs::Convert(_now);
  ...
  msg.set_data(
    ...
    rendering::PixelUtil::MemorySize(rendering::PF_FLOAT32_R, width, height));
  ...
  if (this->HasPointConnections() && this->dataPtr->pointCloudBuffer)
  {
    // Set the time stamp
    *this->dataPtr->pointMsg.mutable_header()->mutable_stamp() = msgs::Convert(_now);
    ...
    if (this->dataPtr->image.Width() != width || this->dataPtr->image.Height() != height)
    {
      this->dataPtr->image = rendering::Image(width, height, rendering::PF_R8G8B8);
    }

    // extract image data from point cloud data
    this->dataPtr->pointsUtil.XYZFromPointCloud(
      this->dataPtr->xyzBuffer, this->dataPtr->pointCloudBuffer, width, height);

    // convert depth to grayscale rgb image
    this->dataPtr->ConvertDepthToImage(
      this->dataPtr->depthBuffer, this->dataPtr->image.Data<unsigned char>(), width, height);

    // fill the point cloud msg with data from xyz and rgb buffer
    this->dataPtr->pointsUtil.FillMsg(
      this->dataPtr->pointMsg, this->dataPtr->xyzBuffer,
...
//////////////////////////////////////////////////
double MultibeamSonarSensor::FarClip() const { return this->dataPtr->depthCamera->FarClipPlane(); }

//////////////////////////////////////////////////
double MultibeamSonarSensor::NearClip() const { return this->dataPtr->near; }

//////////////////////////////////////////////////
bool MultibeamSonarSensor::HasConnections() const
{
  return this->HasDepthConnections() || this->HasPointConnections() || this->HasInfoConnections();
}
